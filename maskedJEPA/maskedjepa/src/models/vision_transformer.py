...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        if masks is not None:
            ...
            print(f"after applying masks: {x.shape}")
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x, mask=masks)
            print(f"after transformer block {i}, x.shape:", x.shape)
            if self.out_layers is not None and i in self.out_layers:
                out_i = self.norm(x)
                print(f"output afer norm at block {i}:", out_i.shape)
                outs.append(out_i)
        ...
        if self.norm is not None:
            x = self.norm(x)
            print("final output after norm:", x.shape)
