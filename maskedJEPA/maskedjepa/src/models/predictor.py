...
class VisionTransformerPredictor(nn.Module):
    ...
    def forward(self, ctxt, tgt, masks_ctxt, masks_tgt, mask_index=1):
        ...
        print("batch size:", B)
        print("ctct shape:", ctxt.shape)
        print("tgt shape:",tgt.shape)
        # Map context tokens to pedictor dimensions
        x = self.predictor_embed(ctxt)
        print("After predictor embd ctxt", x.shape)
        _, N_ctxt, D = x.shape
        print("N_ctxt", N_ctxt, "D:", D)
        ...
        if self.mask_tokens is None:
            ...
            print("before diffcusion pred_tokens", pred_tokens.shape)
            pred_tokens = self.diffusion(pred_tokens)
            print("after diffusion pred_tokens:", pred_tokens.shape)
        else:
            ...
            print("using mask tokens, pred_tokens shape", pred_tokens.shape)
        ...
        if self.predictor_pos_embed is not None:
            ...
            print("positional embedding added to the target tokens",pred_tokens.shape)

        # Concatenate context & target tokens
        x = x.repeat(len(masks_tgt), 1, 1)
        x = torch.cat([x, pred_tokens], dim=1)
        print("concatenated context and targets", x.shape)
        ...
        print("after predicotr_norm:", x.shape)

        # Return output corresponding to target tokens
        x = x[:, N_ctxt:]
        print("after slicing target tokens from x:", x.shape)
        x = self.predictor_proj(x)
        print("final output shape after projection:", x.shape)
